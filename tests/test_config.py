...
def test_set_warning_above_default_critical_rejected(tmp_path: Path):
    """Defaults: warning=None→65, critical=None→85. Setting warning=90
    would create an invalid pair (90 >= 85) and crash every render."""
    p = tmp_path / "cfg.json"
    with pytest.raises(ValueError, match="invalid"):
        cfg_mod.set_value("warning_threshold", "90", p)


def test_set_critical_below_warning_rejected(tmp_path: Path):
    """If warning is already 50, setting critical=40 is invalid."""
    p = tmp_path / "cfg.json"
    cfg_mod.set_value("warning_threshold", "50", p)    # 50 < 85 OK
    with pytest.raises(ValueError, match="invalid"):
...
def test_thresholds_can_be_widened_in_correct_order(tmp_path: Path):
    ...
    p = tmp_path / "cfg.json"
    cfg_mod.set_value("critical_threshold", "90", p)  # 65 < 90 OK
...
def test_show_cache_age_defaults_false_when_key_missing(tmp_path: Path):
    p = tmp_path / "cfg.json"
